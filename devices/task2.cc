...
int main(int argc, char* argv[]) {
  ...
  double MIM26 = 70e-3 / X0_Si;
  double SIL = 700e-3 / X0_Si;
  ...
  plane material(2 * DIST + DUT_DIST, SIL, false);
  datura.push_back(material);
...
//
    ...
    LOG(logRESULT) << "Track resolution at Scatterer with " << 700 << " " << mytel.getResolution(3);
    LOG(logRESULT) << "Track resolution at First with "  << mytel.getResolution(0);
    LOG(logRESULT) << "Track resolution at Last with " << mytel.getResolution(6);
